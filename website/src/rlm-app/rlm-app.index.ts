...
import * as view from './rlm-app.template.html';
import {customElement, property, observe} from '@polymer/decorators';
import {RlmHub} from '../models/rlm-hub';
...
import {AppDrawer, PaperDialog, PaperInput} from '@webcomponents/webcomponentsjs';

@customElement("rlm-app")
export class RlmApp extends PolymerElement {

    @property({ type: Object, notify: true })
    hub: RlmHub;

    @property({ type: Object, reflectToAttribute: true, notify: true })
    ...
    routeTournamentIdChanged(newId: number, oldId: number) {
        ...
        let drawer: AppDrawer = this.$.drawer;
        drawer.close();
    }
    ...
    signInTap(): void {
        let signInDialog: PaperDialog = this.$.signInDialog;
        signInDialog.open();
    }

    doSignIn(): void {
        // let request: any = this.$.authAjax;
        // let authName: PaperInput = this.$.authName;
        // let authPassword: PaperInput = this.$.authPassword;
        // request.body = { "name": authName.value, "password": authPassword.value };
    ...
    authError(e: any): void {
        let authErrorMessage: HTMLDivElement = this.$.authErrorMessage[0];
        authErrorMessage.innerText = "Unable to sign-in.";
    }
